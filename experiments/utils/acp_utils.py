...
def set_backend_model(name: str, **kwargs) -> None:
    ...
    # ensure the flag exists with a sensible default
    kw = dict(kwargs)
    kw.setdefault("refit_each_step", False)   # heavy models fit once by default
...
#         print(f"[ACP] Step {i+1}/{test_size}, alpha_t={alpha_t:.4f}", end="\r")
...
def _run_acp_with_reg(
    ...
    for i in range(test_size):
        ...
        if (not refit_each and i == 0) or refit_each:
            reg.fit(X_win[idx_tr, :], y_win[idx_tr], args)
...
def _run_multi_gamma_with_reg(
    ...
    for k, g in enumerate(gammas):
        _, up = _run_acp_with_reg(Xd, y, alpha, float(g), train_size, ctor, dict(kw), args)
        Q[k, :] = up
...
def aci_intervals(
    ...
    if _BACKEND.get("basemodel") in {"RF", "OLS"}:
        ...
        return _run_acp_with_reg(_BACKEND["X"], _BACKEND["Y"], alpha, gamma, train_size, ctor, _BACKEND["custom_kwargs"], args)
    else:
...
def agaci_ewa(
    ...
    if _BACKEND.get("basemodel") in {"RF", "OLS"}:
        ...
        return _run_multi_gamma_with_reg(_BACKEND["X"], _BACKEND["Y"], alpha, train_size, gammas, eta, args)
    else:
